...
function BalanceBadge(props) {
  ...
  if (usage && usage.ok && usage.enabled !== false) {
    var parts = [];
    var dsh = usage.bySource && usage.bySource.dsh;
    if (dsh && dsh.sessionCount > 0) {
      var dt = dsh.input + dsh.output + dsh.cache; // output 已含 reasoning，不重复计
      var ds = "DSH " + formatTokens(dt) + " tok";
      if (usage.cost !== undefined) {
        ds += " · ¥" + usage.cost.toFixed(2);
        if (usage.costPeak !== undefined && usage.costPeak > usage.cost) {
          ds += "~" + usage.costPeak.toFixed(0);
        }
      }
      parts.push(ds);
    }
    var codex = usage.bySource && usage.bySource.codex;
    if (codex && codex.sessionCount > 0) {
      parts.push("Codex " + formatTokens(codex.input + codex.output + codex.cache) + " tok");
    }
    var claude = usage.bySource && usage.bySource.claude;
    if (claude && claude.sessionCount > 0) {
      parts.push("Claude " + formatTokens(claude.input + claude.output + claude.cache) + " tok");
    }
    if (parts.length > 0) {
      items.push(React.createElement("span", { key: "usage", className: "dsh-balance-item is-usage" }, parts.join(" · ")));
    }
  }
...
function CredentialSection() {
  ...
  var inputsPair = React.useState({});
  var inputs = inputsPair[0];
  var setInputs = inputsPair[1];
  var msgPair = React.useState("");
  var msg = msgPair[0];
  var setMsg = msgPair[1];

  function refresh() {
    fetch("/dsh-balance/api/credentials")
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (d) { if (d && d.providers) setList(d.providers); })
      .catch(function () {});
  }
  ...
  function saveCred(id, ref) {
    var value = inputs[id] || "";
    if (value.trim() === "") { setMsg("请输入 key"); return; }
    setMsg("");
    fetch("/dsh-balance/api/credentials", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ ref: ref, value: value }),
    })
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (d) {
        if (d && d.ok) { setMsg("已保存"); setInput(id, ""); refresh(); }
        else { setMsg(d && d.error ? d.error : "保存失败"); }
      })
      .catch(function () { setMsg("保存失败"); });
  }

  if (list === null) {
    return React.createElement("div", null, "凭据加载中…");
  }
  var rows = list.map(function (p) {
    var input = React.createElement("input", {
      type: "password",
      placeholder: p.apiKeyRef,
      value: inputs[p.id] || "",
      autoComplete: "off",
      onChange: function (e) { setInput(p.id, e.target.value); },
    });
    var status = p.configured
      ? React.createElement("span", { className: "dsh-balance-saved" }, "✓ 已配置")
      : React.createElement("span", { className: "dsh-balance-item is-error" }, "未配置");
    return React.createElement("div", { key: p.id, className: "dsh-balance-provider" },
      React.createElement("span", null, p.name),
      status,
      input,
      React.createElement("button", { onClick: function () { saveCred(p.id, p.apiKeyRef); } }, "保存")
    );
  });
  return React.createElement("div", null,
    React.createElement("div", { className: "dsh-balance-subtitle" }, "API Key 凭据"),
    rows,
    msg ? React.createElement("span", { className: "dsh-balance-saved" }, msg) : null
  );
}
...
function SettingsPanel(props) {
  ...
  return React.createElement("div", { className: "dsh-balance-panel" },
    ...
    React.createElement("label", { className: "dsh-balance-row" },
      React.createElement("input", {
        type: "checkbox",
        checked: config.usageEnabled !== false,
        onChange: function (e) { update({ usageEnabled: e.target.checked }); },
      }),
      React.createElement("span", null, "启用项目用量统计（token/金额）")
    ),
    React.createElement("div", null,
      React.createElement("div", { className: "dsh-balance-subtitle" }, "提供商"),
      rows
    ),
    React.createElement(CredentialSection),
    React.createElement("div", null,
