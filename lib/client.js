...
function BalanceBadge(props) {
  ...
  if (usage && usage.ok && usage.enabled !== false) {
    ...
    if (dsh && dsh.sessionCount > 0) {
      var dt = dsh.input + dsh.output + dsh.cache + dsh.reasoning;
      var ds = "DSH " + formatTokens(dt) + " tok";
      if (usage.cost !== undefined) ds += " · ¥" + usage.cost.toFixed(3);
      parts.push(ds);
    }
    var codex = usage.bySource && usage.bySource.codex;
    if (codex && codex.sessionCount > 0) {
      parts.push("Codex " + formatTokens(codex.input + codex.output + codex.cache + codex.reasoning) + " tok");
    }
    var claude = usage.bySource && usage.bySource.claude;
    if (claude && claude.sessionCount > 0) {
      parts.push("Claude " + formatTokens(claude.input + claude.output + claude.cache + claude.reasoning) + " tok");
    }
    if (parts.length > 0) {
